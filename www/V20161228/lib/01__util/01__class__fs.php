...
class fs{
	...
	static function dirNameInfos($path){
		$arrInfos = explode('__', fs::pathLastPart($path));
		if(count($arrInfos)<2 ){return false;}
		$assocArrInfos=array();
		for($i=0; $i<count($arrInfos); $i+=2){ $assocArrInfos[$arrInfos[$i]]=$arrInfos[$i+1]; }
...
*/	
	static function pathLastPart($path, $removeExtension = null){
		...
		//$name = preg_replace('~.*'.preg_quote(DIRECTORY_SEPARATOR).'~','',$name);
		$name = basename($name);
