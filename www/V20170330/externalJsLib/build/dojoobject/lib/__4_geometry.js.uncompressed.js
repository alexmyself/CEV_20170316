...
ItemContainerInnerGeometry=function(selector, overridingValues){
	...
	tmpObj.itemContainerWI=itemContainerNode.style('width');
	tmpObj.itemContainerHI=itemContainerNode.style('height');


	//Border are a problem to correctly math things, so switch on before anything.
	...
	//Items could have been hidden, so make them visible again
	itemContainerNodesList.children().style('display', '');
	...
	if(labelNodesList.length > 0){
		...
		tmpObj.labelContainerWI=geoHelper.get.wI(labelNode);

		geoHelper.set.wO(labelNodesList.children('.border, .background, .backgroundHover'), tmpObj.labelContainerWI);
		geoHelper.set.wO(labelNodesList.children('.label'), geoHelper.get.wI(labelNodesList.children('.border').first()) );

		//Text could have been set to a 'max', resets it's effect.
		labelNodesList.children('.label').style('height','');

		sizeText(labelNodesList.children('.label'), values.labelTextH*tmpObj.labelContainerWI);
		var max= geoHelper.get.max.h(labelNodesList.children('.label') );
		...
		if(max > (coef*tmpObj.itemContainerHI)){ 
			...
			geoHelper.set.hO(labelNodesList.children('.label'), max);
	...
	if(imageNodesList.length > 0){
		...
		geoHelper.set.t(imageNodesList, geoHelper.get.b(labelNode) + mT );
